...
Процедура ПредставлениеНачалоВыбораЗавершение(Знач РезультатЗакрытия, Знач ДополнительныеПараметры) Экспорт
	...
	Если НоваяСтрока.Свойство("ХранитьИсториюИзменений") И НоваяСтрока.ХранитьИсториюИзменений Тогда
		...
		Если НайденныеСтроки.Количество() > 1 Тогда
			...
			СтрокаСПервоночальнымАдресом = Неопределено;
			...
			Если ЗначениеЗаполнено(МинимальнаяДата) Тогда
				...
				СтрокиСМинальнойДатой = КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(Отбор);
				Если СтрокиСМинальнойДатой.Количество() > 0 Тогда
					СтрокиСМинальнойДатой[0].ДействуетС = Дата(1, 1, 1);
				КонецЕсли;
			КонецЕсли;
